#!/usr/bin/env python3
"""RazorStrike-v2 Merge Script - DARE-TIES on Qwen3.6-35B-A3B.

Anchor: Qwen/Qwen3.6-35B-A3B (base, all verbatim-keep tensors sourced here)
Donors (order matters - donor[0] is the ROUTER source + dominant weight,
per the original merge plan's Step 4 "Router & Dynamics Patching": router
copied verbatim from the dominant donor, overriding whatever DARE-TIES produced):
  0. huihui-ai/Huihui-Qwen3.6-35B-A3B-Claude-4.7-Opus-abliterated
     - reasoning (contains the lordx64 Opus-Reasoning-Distilled delta) + UNCENSORED (abliterated)
  1. AlexWortega/SIQ-1-35B (text-only donor - no vision/MTP keys, harmless: those
     tensors are anchor-keep/verbatim-copy families, never reach the donor-merge path)
     - agentic + coding + autoresearch
  2. UnipatAI/UniMath-35B-A3B
     - olympiad math / proof-synthesis

Recipe locked with the user 2026-07-18 (uncensored variant of the researched recipe):
  result = anchor + DARE_TIES({huihui: 0.45/d0.55, SIQ-1: 0.40/d0.50, UniMath: 0.25/d0.45})
on mergeable tensors only (full-attn q/k/v/o, linear-attn in_proj_qkv/z/out_proj,
routed expert gate_up_proj/down_proj, shared_expert{,_gate}). Router (mlp.gate) verbatim
from donor 0 (huihui). All norms/embed/lm_head/SSM-dynamics/vision/MTP verbatim from anchor.

This is the SAME categorize_tensors/dare_ties_merge/apply_dare_ties/streaming-shard-loop
logic already proven in merge_qwen_moe.py (shipped as razorstrike-v1) - only the
anchor/donor paths, weights, and densities differ for this run.
"""
import json
...
_DEFAULT_DONOR_PATHS = [
    "/Volumes/Scratch/ml-workspace/models/huihui-Opus-abliterated",
    "/Volumes/Scratch/ml-workspace/models/SIQ-1-35B",
    "/Volumes/Scratch/ml-workspace/models/UniMath-35B-A3B",
]
_DEFAULT_OUTPUT_PATH = "/Volumes/Scratch/ml-workspace/merged/razorstrike_v2_bf16"
# Locked recipe: huihui dominant (reasoning + uncensored), SIQ-1 agentic/coding, UniMath math.
WEIGHTS = [0.45, 0.40, 0.25]
DENSITIES = [0.55, 0.50, 0.45]
DENSITY = 0.5  # fallback default if a donor index has no entry


def categorize_tensors(all_keys: set) -> Tuple[set, set, set, dict, set]:
    """Categorize tensors into router, anchor-keep, mergeable, and verbatim-copy families."""
...
def merge_qwen_moe(anchor_path: str = _DEFAULT_ANCHOR_PATH,
    ...
    print("=== RazorStrike-v2 Merge (DARE-TIES on Qwen3.6-35B-A3B) ===")
    ...
    print("\n=== Merge Summary ===")
    print(f"  Router tensors (verbatim from donor 1 = huihui-Opus-abliterated): {len(router_tensors)}")
    print(f"  Anchor-keep tensors (verbatim from anchor): {len(anchor_keep_tensors)}")
    print(f"  Verbatim-copy tensors (vision/MTP): {len(verbatim_copy_tensors)}")
    print(f"  Mergeable tensors (DARE-TIES): {len(mergeable_tensors)}")
    print(f"  Total output tensors: {total_processed}")
