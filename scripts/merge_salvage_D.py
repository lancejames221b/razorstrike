#!/usr/bin/env python3
"""RazorStrike-v3 salvage attempt: DARE-TIES on Qwen3.6-35B-A3B, all 3 donors, with
UniMath heavily sparsified/de-weighted to test whether the CJK-contamination
directions in its task vector can be masked out while retaining some math benefit.

Root cause (confirmed via pairwise bisect 2026-07-19): UniMath's task vector, when
combined with EITHER huihui's OR SIQ-1's delta via DARE-TIES, produces severe
coding-context CJK contamination - even though UniMath alone (full weights) is
clean. huihui+SIQ-1 alone (no UniMath) is clean. This means the contaminating
signal is a SUBSET of UniMath's delta dimensions, exposed only through TIES
sign-election combination.

Salvage strategy: density 0.45->0.15 (survive only ~1/3 as many delta elements,
random per-tensor-per-seed) + weight 0.25->0.15 (less voting power in sign
election). huihui and SIQ-1 unchanged (0.45/0.55, 0.40/0.50) since pairwise
bisect already proved them clean together at those exact values.
"""
import json
...
_DEFAULT_OUTPUT_PATH = "/Volumes/Scratch/ml-workspace/merged/salvage_E_bf16"
# Salvage E (escalated): huihui/SIQ-1 unchanged, UniMath even more aggressively
# sparsified+de-weighted than salvage D (0.15/0.15 -> 0.08/0.08) after D still
# showed light contamination in 5/6 tests.
WEIGHTS = [0.45, 0.40, 0.08]
DENSITIES = [0.55, 0.50, 0.08]
...
def merge_qwen_moe(anchor_path: str = _DEFAULT_ANCHOR_PATH,
    ...
    print("=== RazorStrike-v3 Salvage E: 3-donor, UniMath sparsified (d0.08/w0.08) ===")
